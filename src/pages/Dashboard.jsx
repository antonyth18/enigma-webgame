...
import { DeadTree, RadioTower } from '../components/icons/index.jsx';
...
export default function App() {
  ...
  const handleCorrectAnswer = () => {
    ...
    const newStages = stages.map((stage) => {
      ...
      // Check if this stage should be unlocked based on new points
      if (newTotalPoints >= stage.unlockPoints && stage.questions.every((q) => q.status === 'locked')) {
        // Unlock all questions in this stage
                  ...
                  {!isUnlocked && (
                    <div className="absolute inset-0 bg-black/70 backdrop-blur-sm rounded-xl flex items-center justify-center pointer-events-none">
                      <div className="text-center">
                        <div
                          className="w-16 h-16 rounded-full bg-[var(--ash-darker)]/40 border-2 border-[var(--ash-darker)] flex items-center justify-center mx-auto mb-3"
                        >
                          <svg
                            width="32"
                            height="32"
